...
{
public:
	Movement_Class() :Is_Circular(false) {}
	~Movement_Class() = default;

	//根据目标坐标和插补参数插补路径
	bool Init(const Coordinate_Class& Origin_Coor, const Interpolation_Parameter_TypedefStructure& Input_Para);
	//根据当前坐标计算目标坐标，目标速度，返回计算结果
	bool Cal_Velocity(const Coordinate_Class Current_Coor_InWorld);

	void Set_Destination(const Coordinate_Class& Destination, const float threshold) {
		Destination_Coor_InWorld = Destination;
		this->threshold = threshold;
	}	//设置终点
	Coordinate_Class& Get_Destination(void) { return Destination_Coor_InWorld; }

	static Velocity_Class Target_Velocity_InAGV;	//目标速度
	static Coordinate_Class Target_Coor_InWorld;	//目标坐标

protected:
	bool Is_Circular;	//指示当前运动是圆周运动
	...
	virtual Velocity_Class& Assign_Velocity(const Coordinate_Class& Current_Coor_InOrigin, const float velocity) = 0;	//根据期望坐标在起点坐标中的坐标，将总速度分配给各个轴
